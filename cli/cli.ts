...
import EnumNamingStrategy from '../lib/enumNamingStrategy';
import type { EnumType } from '../lib/generateEnum';
import processInputs from '../lib/process';
import PropertyNamingStrategy from '../lib/propertyNamingStrategy';
...
class TypeScriptEnumGeneratorAction extends CommandLineAction {
  ...
  constructor() {
    ...
    this._followLinks = this.defineFlagParameter({
      description:
        'If a URL is supplied, and that URL returns Bundle resources, setting this flag instructs the program that it should attempt to follow "nexT links in the Bundle (iteratively, until none are returned).',
    ...
    this._inputFilePattern = this.defineStringParameter({
      argumentName: 'GLOB',
      description:
        'A glob pattern that should resolve to one or more files containing either an expanded ValueSet or a Bundle of expanded ValueSets.',
    ...
    this._outputDirectory = this.defineStringParameter({
      argumentName: 'OUTDIR',
      description:
        'A path to which the generated enum files should be written. If omitted, the generated output will be printed to stdout',
    ...
    this._enumNamingStrategy = this.defineChoiceParameter<'simple' | 'version_aware'>({
      ...
      description:
        'The strategy for naming the generated enums. The default is "simple", which uses the "name" property of the ValueSet. The other option is "version_aware", which suffixes the generated enum with the FHIR version, obtained from the ValueSet definition (e.g. "IssueSeverityR5").',
    });
